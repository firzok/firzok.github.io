// @ts-nocheck
// Ported ambient FX: starfield canvas, custom cursor, scroll reveal, and the
// asteroid-on-timeline animation. Loosely typed on purpose — this is DOM
// choreography, not app logic.
...
function updateAndDrawShootingStars(now, palette) {
  ...
  for (let i = shootingStars.length - 1; i >= 0; i -= 1) {
    ...
    if (star.life > star.maxLife || star.x > window.innerWidth + 140 || star.y > window.innerHeight + 80) {
...
function initCursor() {
  ...
  window.addEventListener("mouseenter", () => (pointer.active = true));
}
...
function getItemCheckpointOffset(item) {
  ...
  if (meta) return item.offsetTop + meta.offsetTop + Math.min(24, meta.offsetHeight * 0.5);
...
function updateTimelineGeometry() {
  ...
  const endPx = getTimelineEndOffset();
  timelineTravelPx = Math.max(0, endPx - timelineStartPx);
...
function updateTimelineProgress() {
  ...
  const nearestDistance = timelineDotOffsets.reduce((min, dotY) => Math.min(min, Math.abs(markerY - dotY)), Infinity);
...
function animateTimelineState(force = false) {
  ...
  if (Math.abs(timelineProgressTarget - timelineProgressCurrent) < 0.0002) timelineProgressCurrent = timelineProgressTarget;
  if (Math.abs(timelineImpactTarget - timelineImpactCurrent) < 0.0002) timelineImpactCurrent = timelineImpactTarget;
  ...
  const darkTheme = root.getAttribute("data-theme") !== "light";
